...
### 3. Extract Topic Taxonomy (if `prompts/taxonomy/{area}_taxonomy.json` does not exist)

```bash
cd area_summary

# Check if taxonomy exists; if not, run extract_topic_taxonomy skill
if [ ! -f "prompts/taxonomy/{area}_taxonomy.json" ]; then
    # Use the extract_topic_taxonomy.md skill to generate taxonomy
    # This reads background_{area}.txt and creates the taxonomy JSON
    echo "Taxonomy missing — invoke extract_topic_taxonomy.md skill for area: {area}"
fi
```
...
| `prompts/taxonomy/{area}_taxonomy.json` | `extract_topic_taxonomy.md` skill | Skip extraction if exists |
...
open prompt_optimization/area_summaries/{area}/area_summary.html
```
...
# Step 6 can also be run standalone:
