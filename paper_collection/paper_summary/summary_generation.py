...
def call_gemini_api(
    ...
    """
    Call the Gemini API via wearables-ape.io with retry logic.
        ...
        try:
            ...
            if response.status_code in RETRYABLE_STATUS_CODES:
                ...
                if attempt < max_retries:
                    ...
                    print(
                        f"  Retry {attempt + 1}/{max_retries}: HTTP {response.status_code}, waiting {delay}s..."
                    )
                    time.sleep(delay)
                    continue
            else:
            ...
            if attempt < max_retries:
                delay = min(RETRY_BASE_DELAY * (2**attempt), RETRY_MAX_DELAY)
                print(
                    f"  Retry {attempt + 1}/{max_retries}: Timeout, waiting {delay}s..."
                )
                time.sleep(delay)
                continue
            ...
            if attempt < max_retries:
                delay = min(RETRY_BASE_DELAY * (2**attempt), RETRY_MAX_DELAY)
                print(
                    f"  Retry {attempt + 1}/{max_retries}: Connection error, waiting {delay}s..."
                )
                time.sleep(delay)
                continue
...
def load_prompt_template(prompt_file: str = None) -> str:
    ...
    Replaces:
        ...
        - <rag_background> with contents of background_rag.txt
    ...
    if prompt_file is None:
        ...
        prompts_dir = Path(prompt_file).parent
    ...
    # Load and replace <rag_background>
    background_file = prompts_dir / "background_rag.txt"
    if background_file.exists():
        with open(background_file, "r") as f:
            rag_background = f.read()
        prompt = prompt.replace("<rag_background>", rag_background)
...
def load_json_template(template_file: str = None) -> dict:
    ...
    Args:
        template_file: Path to template file. Defaults to prompts/summary_template.json.
    ...
    if template_file is None:
        script_dir = Path(__file__).parent
        template_file = script_dir / "prompts" / "summary_template.json"
...
def generate_summary_for_paper(
    ...
    # ========================================
    # PRE-VALIDATION: Check required columns before calling LLM
    # ========================================
    ...
    if not title or not title.strip():
        ...
        print(f"  Skipping paper ID {paper_id}: no title")
        return result
    ...
    if not link or not link.strip():
        if should_close:
            db.close()
        result["error"] = "Missing required field: link"
        print(f"  Skipping paper ID {paper_id}: no link")
        return result
    ...
    # Check we have a valid arXiv PDF URL
    if not pdf_url or "arxiv.org" not in pdf_url:
        if should_close:
            db.close()
        result["error"] = f"No valid arXiv PDF URL (link: {link[:50]}...)"
