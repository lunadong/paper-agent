...
**Note:** Both local development and production use OpenAI's
`text-embedding-3-small` (512 dimensions) for embeddings and search queries.
The database stores pre-computed embeddings using pgvector.
...
- [x] Browse papers with pagination
- [x] Semantic search (AI-powered, uses OpenAI embeddings)
- [x] Keyword search (SQL ILIKE fallback)
- [x] Topic filtering (multi-select)
...
This connects to the same Neon PostgreSQL database and uses OpenAI API
for embeddings, just like production.
