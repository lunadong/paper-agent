...
let currentPrimaryTopic = '';  // Single primary_topic filter
...
async function fetchPapers(page = 1, search = '') {
    ...
    try {
        ...
        if (currentPrimaryTopic) {
            params.set('primary_topic', currentPrimaryTopic);
        }
        ...
        if (currentPrimaryTopic) {
            url.searchParams.set('primary_topic', currentPrimaryTopic);
        } else {
            url.searchParams.delete('primary_topic');
        }
...
function renderStats(data) {
    ...
    const showLineChart = currentSearch || currentTopics.length > 0 || currentPrimaryTopic;
...
function renderMonthlyChart(monthlyStats) {
    ...
    // Check if any filter is active (including primary_topic)
    const hasActiveFilter = currentSearch || currentTopics.length > 0 || currentPrimaryTopic || currentDateFrom || currentDateTo;
...
// Toggle topic filter (3-state: none → topics → primary_topic → none)
function toggleTopicFilter(topic) {
    // Check if this topic is currently the primary_topic filter
    if (currentPrimaryTopic === topic) {
        // Third click: clear all (go back to none)
        currentPrimaryTopic = '';
        // Also remove from topics if somehow present
...
function updateTopicButtons() {
    ...
    buttons.forEach(btn => {
        ...
        if (match) {
            ...
            const fullName = topicFullNames[btnTopic] || btnTopic;

            // Check if this topic is the primary_topic filter
            if (currentPrimaryTopic === btnTopic) {
                btn.classList.add('primary-active');
                btn.classList.remove('active');
                btn.setAttribute('data-tooltip', 'unfilter');
            } else if (currentTopics.includes(btnTopic)) {
                btn.classList.add('active');
                btn.classList.remove('primary-active');
                btn.setAttribute('data-tooltip', `primary area: ${fullName}`);
            } else {
                btn.classList.remove('active', 'primary-active');
                btn.setAttribute('data-tooltip', fullName);
            }
        }
    });
}
...
window.onpopstate = function() {
    ...
    currentPrimaryTopic = params.get('primary_topic') || '';
